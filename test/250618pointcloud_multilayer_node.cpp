                           ...
                           const nav_msgs::OdometryConstPtr &odom)
{
    ...
    // std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> segmented_ground_clouds(4);
    // for(int i=0; i<4; ++i) {
    //     segmented_ground_clouds[i].reset(new pcl::PointCloud<pcl::PointXYZ>());
    // }

    // 在更新非地面点之前，先用一种快速、直接的方式处理地面
    if (!ground_cloud.empty())
    {
        // 将地面点云从传感器坐标系转换到世界坐标系
        pcl::PointCloud<pcl::PointXYZ> ground_cloud_world;
        ground_cloud_world.reserve(ground_cloud.points.size());
        for (const auto& point : ground_cloud.points)
        {
            ...
            world_point.x = pt_world(0);
            world_point.y = pt_world(1);
            world_point.z = pt_world(2);

            // if (std::abs(angle) <= rad_45) { // 前方
            //     segmented_ground_clouds[0]->points.push_back(world_point);
            // } else if (angle > rad_45 && angle <= rad_135) { // 左方
            //     segmented_ground_clouds[1]->points.push_back(world_point);
            // } else if (angle < -rad_45 && angle >= -rad_135) { // 右方
            //     segmented_ground_clouds[2]->points.push_back(world_point);
            // } else { // 后方
            //     segmented_ground_clouds[3]->points.push_back(world_point);
            // }
            
            ground_cloud_world.points.push_back(world_point);
        }
        
        if (!ground_cloud_world.empty())
        {
            // 获取地图锁，因为我们将要调用地图更新方法
            std::lock_guard<std::mutex> lock(map_mutex_);
            map_.updateGroundBlocks(ground_cloud_world);
        }
    }
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        for (const auto& point : current_cloud->points)
        {
            ...
            if (pt_c.norm() >= camData_.depth_mindist && pt_c.norm() <= camData_.depth_maxdist)
            {
                ...
                ptws_hit.points.emplace_back(pt_w(0), pt_w(1), pt_w(2));
            }
        }
        if (ptws_hit.empty()) continue;

        // pcl::PointCloud<pcl::PointXYZ>::Ptr current_ground_cloud = segmented_ground_clouds[i];
        // pcl::PointCloud<pcl::PointXYZ> ptws_miss = *current_ground_cloud;
