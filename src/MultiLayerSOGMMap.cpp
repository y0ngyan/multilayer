...
double SOGMMap::getAdaptiveValidRatio(double distance, double normalized_std_dev) const {
    // 表面一致性因子
    double consistency_factor = std::exp(-consistency_factor_ * normalized_std_dev);
    // 距离因子
    double distance_factor = std::exp(-distance_decay_rate_ * distance);
    double final_ratio = valid_ratio_min_ + (valid_ratio_base_ - valid_ratio_min_) * consistency_factor * distance_factor;
    final_ratio = std::min(final_ratio, 1.0); // 确保不超过1.0
    return std::max(final_ratio, MIN_VALID_RATIO_RATIO_);
}
...
bool SOGMMap::getDepthInterval(const cv::Mat &depth_image,
    ...
    // 计算采样步长，确保至少采样一些点
    int target_sample_count = static_cast<int>(total_pixels_in_patch * sampling_ratio);
    target_sample_count = std::max(target_sample_count, 4); // 至少采样9个点（3x3网格）
    ...
    // 根据valid_depths计算统计量
    if (total_depths.size() > 4) {
        // 计算最小值和最大值
        auto minmax_it = std::minmax_element(total_depths.begin(), total_depths.end());
        double min_valid_depth = *minmax_it.first;
        double max_valid_depth = *minmax_it.second;
        
        // 计算平均值
        double sum = std::accumulate(total_depths.begin(), total_depths.end(), 0.0);
        double mean = sum / total_depths.size();
        
        // 计算标准差
        double sq_sum = std::inner_product(total_depths.begin(), total_depths.end(), 
                                         total_depths.begin(), 0.0);
        double std_dev = std::sqrt(sq_sum / total_depths.size() - mean * mean);

        double ratio = static_cast<double>(valid_pixels) / total_pixels;
        if (ratio > 0 && height > 1.5){
            ROS_INFO("Total Depths: Min = %f, Max = %f, Mean = %f, Std Dev = %f, Valid Pixel Ratio = %f, Total_pixels_in_patch = %d, Height = %f",
                 min_valid_depth, max_valid_depth, mean, std_dev, ratio,
                 total_pixels_in_patch, height);
        }
